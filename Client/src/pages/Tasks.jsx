...
const Tasks = () => {
    ...
    useEffect(() => {
        const getTodos = async () => {
            try {
                const response = await getTodo('Tasks');
    ...
    return (
        <div>
            ...
            <div className='flex gap-2 sm:gap-4'>
                ...
                <div className='flex-1 min-w-0 p-4 sm:p-6 md:p-10 flex flex-col gap-4 rounded-l-3xl shadow-[-4px_0px_8px_3px_rgba(0,0,0,0.15)] h-screen overflow-y-auto'>
                    <div className='flex flex-col gap-2'>
                        <h1 className='font-bold text-2xl sm:text-3xl md:text-4xl'>Tasks</h1>
            ...
            {form && (
                <TodoForm setForm={setForm} setTodos={setTodos} category="Tasks" />
            )}

            {editForm && editTodo && (
                <TodoForm setForm={setEditForm} setTodos={setTodos} editTodo={editTodo} onEditSubmit={handleEditSubmit} category="Tasks" />
            )}
        </div>
    )
}
